...
class LikedHousesRecyclerViewAdapter(private val houseList:ArrayList<HouseModel>, private val listener:Listener) : RecyclerView.Adapter<LikedHousesRecyclerViewAdapter.HouseHolder>(){
    interface Listener{
    ...
    class HouseHolder(view: View) : RecyclerView.ViewHolder(view) {
        fun bind(houseModel: HouseModel, colors:Array<String>, position: Int, listener:Listener){
            ...
            itemView.likedHousesRecyclerViewHouseAddress.text = "Adres : " + houseModel.houseAddress
            itemView.likedHousesRecyclerViewRent.text = "Kira : " + houseModel.rent.toString()
            itemView.likedHousesRecyclerViewHeatResource.text ="Yakıt Türü : " + houseModel.heatResource

        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HouseHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.liked_houses_row,parent,false)
